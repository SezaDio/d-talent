 ...
 */
function srv_send_changePassword_email($idMember, $emailAddr, &$errorMessage, $createNew = false) {
	$CI =& get_instance();
	...
	if (!empty($emailRecord->date_last_attempt)) {
		...
		if ($timeDiff < ($validMinute * 60)) {
			$errorMessage = "Anda hanya dapat mengirimkan e-mail forgot password setiap {$validMinute} menit sekali.".
	...
	if ($userData->role == 'talent') {
		$accessLink = site_url('/AccountTalent/changePass/'.$accessKey.'?em='.$encEmail);
	}else if($userData->role == 'company'){
		$accessLink = site_url('/AccountCompany/changePass/'.$accessKey.'?em='.$encEmail);
	}
	
	$emailSubject = "[D-Talent] Reset Password";
	...
	$bodyContent = $CI->load->view('email/email_forgot_password', $contentFields, true);
	$contentFields['pageContent'] = $bodyContent;
	$emailContent = $CI->load->view('skin/email/template_default', $contentFields, true);

	$sendResult = $CI->email_model->web_send_email($emailAddr, $emailSubject, $emailContent, 'FORGOT-PASSWORD', false);
	if ($sendResult) {
